...
\section{The 4.0 Certificate (Master's Head Start)}
\newthought{A Master's program may demand proof of a passed thesis} long before your official documents exist. The DHBW issues them only around mid-November, but Master's enrollment often needs proof in summer. The bridge is the so-called 4,0-Bescheinigung: a certificate confirming your thesis will pass with at least a grade of 4.0.
\marginnote{\textbf{Timeline gap:} degree documents are ready mid-November; Master's enrollment wants proof in summer.}

\newthought{The process runs in three steps.} First, request the certificate from the Prüfungsamt via the function mailbox, using the prescribed subject line. They reply with a fee notice and attach the confirmation form. Second, pay the fee (currently 20~€) and send a payment screenshot by mail. Third, return the confirmation form, signed \emph{in original ink} by your examiner(s), either in person or by post.
\marginnote{\textbf{Contact:} \href{mailto:pruefungsamt@dhbw-ravensburg.de}{pruefungsamt@dhbw-ravensburg.de}}
\marginnote{\textbf{Subject line:} \texttt{Bescheinigungsanfrage\_Name [Nachname], Vorname [Vorname]}}

\newthought{The confirmation form comes from the Prüfungsamt.} You do not source it yourself; it arrives attached to their reply. Fill in your details, then have it signed by your examiner(s). If a second examiner exists, both sign; if there is only one examiner, the first examiner's signature suffices.
\marginnote{\textbf{Original signatures only:} the form must not be copied or scanned.}

\newthought{Precondition:} the thesis must already be submitted to the examiners and the Prüfungsamt before the form can be signed. The certificate is issued once the fee is paid and the original form is on file.
